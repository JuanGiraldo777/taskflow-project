/**
 * @file server_backend/scripts/updateDescriptions.js
 * @description Carga las descripciones de los productos ORIGINALES (el Excel
 * de origen no traía esa columna, así que se escribieron a mano, una por
 * producto, evitando la fórmula genérica "Una fragancia X con notas de Y").
 *
 * Por defecto corre en modo DRY-RUN (solo muestra qué cambiaría).
 * Para escribir de verdad: node scripts/updateDescriptions.js --commit
 */
const pool = require("../src/config/db");

const COMMIT = process.argv.includes("--commit");

// Clave = id real en `products` (no el nombre — el nombre puede repetirse
// entre presentaciones/marcas parecidas y prestarse a error).
const DESCRIPTIONS = {
...
async function main() {
  ...
  const missing = rows.filter((r) => !DESCRIPTIONS[r.id]);
  if (missing.length > 0) {
    console.error(
      `Faltan descripciones para ${missing.length} producto(s):`,
    );
    missing.forEach((r) => console.error(`  id=${r.id} ${r.name}`));
    process.exitCode = 1;
    await pool.end();
    return;
  }
  ...
  for (const row of rows) {
    const description = DESCRIPTIONS[row.id];
    if (!COMMIT) {
      console.log(`  [dry-run] #${row.id} ${row.name} -> "${description.slice(0, 60)}..."`);
      updated += 1;
      continue;
    }
    await pool.execute("UPDATE products SET description = ? WHERE id = ?", [
      description,
      row.id,
    ]);
    updated += 1;
  ...
  console.log(`\n${updated} descripciones ${COMMIT ? "actualizadas" : "listas para actualizar"}.`);
  await pool.end();
}
